...
const filePath = './data/DosageForm.csv';
...
const main = async () => {
    try {        // Connects to the remote mongoDB server
        ...
        // Reads the input CSV file: DosageForm.csv
        let workbook = XLSX.readFile(filePath, { cellDates: true });
        // Dosage Form Entries - array of objects
        let DosageFormEntries = XLSX.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]]);
        ...
        // DosageForms to insert into MongoDB
        DosageFormEntries = DosageFormEntries.map(DosageFormEntry => {
            return constructData(DosageFormEntry, userIds);
        });
        await db.collection('dosageform').insertMany(DosageFormEntries);
        console.log('Dosage Forms created successfully');
        client.close();
    } catch (error) {
        console.log(error);
    }
...
const constructData = (data, userIds) => {
    return {
        ...
        dosageform_id: data.DosageFormID,
        name: data.DosageForm ? data.DosageForm : '',
