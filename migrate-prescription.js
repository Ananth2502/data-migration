...
const filePath = './data/PrescriptionMedication.csv';
...
const main = async () => {
  try {
    ...
    let Prescriptions = XLSX.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]]);
    ...
    Prescriptions = Prescriptions.map(prescription => {
      return constructData(prescription, userIds, brandIds, classificationIds, dosageformIds, dosagefrequencyIds, dosageunitIds, medicationIds, patientIds, tenantIds);
    });
    await db.collection('prescription').insertMany(Prescriptions);
...
const constructData = (data, userIds, brandIds, classificationIds, dosageformIds, dosagefrequencyIds, dosageunitIds, medicationIds, patientIds, tenantIds) => {
  return {
    ...
    is_ph_visible: (data.IsDeleted && data.IsPhoneNoVisible.toLowerCase() == 't') ? true : false,
